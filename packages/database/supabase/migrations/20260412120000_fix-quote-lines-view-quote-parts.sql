-- Restore quoteLines / quoteLinePrices to support quote-only parts (quotePartId, no itemId).
-- 20260321143847_method-type-migration.sql accidentally used INNER JOIN "item", which
-- excluded those rows and broke loaders that use .single() on quoteLines.
