-- Restore inspection On Hold + idempotent create-inspection-lot invoke inside\n-- complete_job_to_inventory (forked from 20260706182830; keeps COALESCE user guard).\n
-- Fire-and-forget create-inspection-lot with stable outputLotKey for idempotency.
CREATE OR REPLACE FUNCTION invoke_create_job_inspection_lot(
  p_job_id TEXT,
  p_company_id TEXT,
  p_user_id TEXT,
  p_output_lot_key TEXT DEFAULT NULL,
...
BEGIN
  ...
  -- Lot policy is mirrored onto item.requiresInspection during Phase 2.2 compat.
  SELECT COALESCE(
    (
      SELECT p."required"
      FROM "itemInspectionPolicy" p
      WHERE p."itemId" = v_item_id
        AND p."companyId" = p_company_id
        AND p."inspectionType" = 'Lot'
      LIMIT 1
    ),
  ...
  v_output_lot_key := 'job:' || p_job_id
    || ':qty:' || COALESCE(v_quantity_received_to_inventory::TEXT, '0')
    || ':loc:' || COALESCE(p_location_id, '')
    || ':su:' || COALESCE(p_storage_unit_id, '');
  ...
  IF v_requires_inspection AND v_quantity_received_to_inventory > 0 THEN
    PERFORM invoke_create_job_inspection_lot(
      p_job_id,
      p_company_id,
      p_user_id,
      v_output_lot_key,
