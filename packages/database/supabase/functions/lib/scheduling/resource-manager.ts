...
import { computeOperationDurations } from "../operation-time.ts";
    ...
    if (this.activeJobs.length > 0) {
      operations = await this.db
        ...
        .select([
          ...
          "jobOperation.partsPerCycle",
          "jobOperation.timeBasis",
...
function getDurations(operation: BaseOperation): Operation {
  ...
  const result = computeOperationDurations(operation);
  ...
  return {
    ...
    laborDuration: result.laborDuration,
    machineDuration: result.machineDuration
  };
}
