...
async function processToActive(
  ...
  for (const docId of idsToUpdateToActive) {
    await rewireSamplingPlansToActiveInspectionDocument(
      client,
      docId,
      companyId
    );
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  switch (field) {
    case "status": {
      const statusValue = value as (typeof inspectionDocumentStatus)[number];
      if (!inspectionDocumentStatus.includes(statusValue)) {
