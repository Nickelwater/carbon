...
import { ShippingLabelPDF } from "@carbon/documents/pdf";
...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  const stream = await renderToStream(
    <ShippingLabelPDF items={items} labelSize={labelSize} logo={logo} />
  );
  ...
  return new Response(new Uint8Array(body), {
    ...
    headers: {
      ...
      "Content-Disposition": `inline; filename="${company.data?.name ?? "Shipping"} - Shipping Labels.pdf"`
    }
  });
}
